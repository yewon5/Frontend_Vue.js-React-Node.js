<template>
  ...
  <div id="app">
    ...
    <write-page :addMemo="addMemo" :setMode="setMode" v-if="mode == MODE.WRITE"></write-page>
  </div>
</template>
...
// default아닌 export는 import할때 여러개중 1개 불러와야하므로 {}로 묶는다. 
...
export default {
  ...
  methods : {
    setReadMemo(index){ // 메모의 인덱스를 리스트에 전달해서 리스트가 호출
      this.selectedMemo = this.list[index]
      this.mode = MODE.READ // 모드 값이 list일때 read가 나타나도록
    },
    setMode(mode){ // 버튼을 눌렀을때 리스트페이지로 이동
      this.mode = mode;
    },
    addMemo(memo){ // 저장 버튼 누르면 함수가 호출되도록
      this.list.push(memo)
